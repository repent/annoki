class CommentsController < ApplicationController
  ...
  def create
    ...
    @comment.parent_id = (@comment.parent_id.empty? ? nil : @comment.parent_id.to_i)

    respond_to do |format|
      if @comment.save
        format.html { redirect_to @comment.discussion, notice: 'Comment was successfully created.' }
        format.json { render :show, status: :created, location: @comment.discussion }
      else
  ...
  private
    ...
    def comment_params
      params.require(:comment).permit(:discussion_id, :text_id, :content, :data, :parent_id)
    end
end
